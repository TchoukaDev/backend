...
module.exports = createCoreController(
  ...
  ({ strapi }) => ({
    async find(ctx) {
      ...
      const entries = await strapi.db.query("api::boutique.boutique").findMany({
      ...
      const total = await strapi.db.query("api::boutique.boutique").count({});
    ...
    async findBySlug(ctx) {
      ...
      const entry = await strapi.db.query("api::boutique.boutique").findOne({
        where: { slug },
        populate: {
          images: {
      ...
      if (!entry) {
        return ctx.notFound("Boutique non trouvée");
      }
